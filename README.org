...
- =rule=：本地确定性整理，**不调用任何大模型**——把行首干净的枚举词（第X点/一、等）
  归一为编号行（`1. …`），并删除口头填充词（见下）。转写只用语音模型一次推理，也不拉起 llama-server。
...
- 指令词/专用词后校准（全档位自动，确定性）：0.6B 模型听一个指令通常有几
  种稳定“听错”的写法，把它见过的所有写法都配置成该指令的候选，转写后一律
  归一为标准词——**不用录音/不用训练**，按你的实测经验手工维护即可。配置源：
  - 首选 **=funasr-server/command_aliases.json=**（随仓库附带，直接编辑）：
    #+BEGIN_SRC json
    {
      "clear":     ["可丽儿", "可丽尔", "克丽尔", "claer"],
      "发送":       ["发松", "发宋", "法送", "发耸"],
      "org model": ["奥格猫抖", "og model", "o r g model"]
    }
    #+END_SRC
    键 = 标准输出，值 = 你实测的几种识别结果；文件里出现的键会整体替换内置
    默认表（其余键保留内置默认）。每个值可用 dict 形式附加开关：
    ={"forms": ["..."], "fuzzy": false}= 关闭模糊英文匹配。
  - 追加式环境变量 =FUNASR_COMMAND_ALIASES='{"clear": ["克丽尔"]}'= 叠加在
    文件之上。
  匹配做了英文容错：大小写不敏感；逐字母念出的 =C L E A R= / =o r g model=
  会自动合并后再比对（连读的整词写法也能命中）；别名内空格宽松
  （=org  model= 也算）；内置 clear 默认开模糊（编辑距离≤2，claer→clear，
  其它英文单词指令可用 =fuzzy=true= 开启）。中文别名按整词出现即替换。
  附带文件已预置几组可直接编辑的常用词（=org / emacs / funasr=，含常见
  拼错与中文谐音），英文标准词替换进中文句时会自动补空格（伊马克斯→emacs 很强大）。
- 口头语过滤（=none= 以外的档位自动，确定性）：删填充词，即使 ASR 没给标点、垫词
  直接粘在正文里也能处理——纯垫音（嗯/呃/诶）处处可删（官方的**呃**飞猪云→官方的飞猪云）；
  啊/呀/哦 等两用叹词只在确定是停顿/句首垫词时删（**啊**那我们→那我们、版本**啊嗯**而且→版本，而且
  ——分句垫词丛会补成逗号），句中真正表语气的保留（好**的**啊、我说啊，这个不行、三点了啊 不动）；
  整词填充（就是说/怎么说呢/哎呀、句首独立的那个/这个、这个这个叠字、句尾叹词后垫的“火啊**这个**”）
  独立或按规则删；粘连语义的哎呀好疼、就选那个。、拿这个来 保留；中英夹杂 um/uh/hmm/oh 同样处理。
  删除残留的重复标点/空格会顺带清理（，，→，、。，→。）。词表在 =funasr-server/filler_filter.py=，
  想增删改顶部 =_PURE_STALL= / =_AMBIGUOUS= / =_CLAUSE_CONNECTORS= =_DISCOURSE_MARKERS= 等常量即可。
- 局限：=rule= 档无法拆"无标点连读"的枚举列表；口头语过滤偏向删除连读垫词，偶尔会把
  语气叹词一并删掉（属激进档预期），要精确按语义改写仍用 =llm= 档；=llm= 档 0.6B 属
  "尽力而为"，复杂长句的语义修正偶有不到位。要更稳可换更大的 chat GGUF。
